...
// ====================================
// LEGAL ENTITY SUFFIX STRIPPER
// "Rkassa B.V." -> "Rkassa", "Acme Holding Group" -> "Acme"
// Strip iteratief, alleen aan het EIND, alleen als er minstens 1 woord overblijft.
// ====================================
const LEGAL_SUFFIXES = [
  // NL
  'b.v.', 'bv', 'b.v', 'n.v.', 'nv', 'n.v',
  'v.o.f.', 'vof', 'v.o.f', 'c.v.', 'cv', 'c.v',
  'holding', 'beheer', 'groep', 'group', 'holdings',
  // EN
...
export function stripLegalSuffix(name) {
  ...
  while (changed) {
    ...
    for (const suffix of LEGAL_SUFFIXES) {
      // Suffix must be at the end and preceded by whitespace, comma, or other separator.
      if (lower.endsWith(' ' + suffix) || lower.endsWith(',' + suffix) || lower.endsWith(', ' + suffix)) {
        const stripped = result.slice(0, result.length - suffix.length).replace(/[\s,]+$/, '').trim();
        // Veiligheidsklep: behoud minstens 1 letter/cijfer
...
export function findNameInCompetitors(companyName, analysisResults) {
  ...
  const nameStripped = stripLegalSuffix(companyName).toLowerCase().trim();
  ...
  for (const [comp, count] of Object.entries(allCompetitors)) {
    ...
    const compStripped = stripLegalSuffix(comp).toLowerCase().trim();

    // Skip als het exact hetzelfde is, ook als enkel het verschil de legal suffix is.
    // "Rkassa B.V." en "Rkassa" zijn hetzelfde bedrijf.
    if (compLower === nameLower) continue;
    if (compStripped && nameStripped && compStripped === nameStripped) continue;
