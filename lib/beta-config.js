...
// AI Visibility Scan: weekly handmatige limieten per tier (Free 2, Lite 3, Pro 7).
// Pro behoudt daarnaast de automatische wekelijkse cron (los van dit handmatige limiet).
// Admin-emails hebben echte unlimited bypass.

// Weekly handmatige AI Visibility scan limieten per tier.
// Aanpasbaar zonder de canUserScan-logica te raken.
const AI_VISIBILITY_WEEKLY_FREE = 2
const AI_VISIBILITY_WEEKLY_LITE = 3
const AI_VISIBILITY_WEEKLY_PRO = 7
...
export const BETA_CONFIG = {
  TOOLS: {
    'ai-visibility': {
      ...
      limits: {
        anonymous: 1,
        authenticated: AI_VISIBILITY_WEEKLY_FREE, // Free baseline; tierLimits overschrijft per tier
        beta_unlimited: true
      },
      // Per-tier weekly limits (Lite/Pro krijgen geen unlimited bypass meer).
      tierLimits: {
        free: AI_VISIBILITY_WEEKLY_FREE,
        lite: AI_VISIBILITY_WEEKLY_LITE,
        pro: AI_VISIBILITY_WEEKLY_PRO,
      },
      cooldown: {
        anonymous: 3600,
        authenticated: 0        // Geen tijd-cooldown, alleen weeklimiet
      },
      resetPeriod: 'weekly'
    },
    'geo-optimalisatie': {
...
export async function canUserScan(supabase, userId, toolName, ipAddress) {
  ...
  if (userId) {
    ...
    // ✨ ADMIN BYPASS - allowed=true ongeacht het verbruik. Bij tools met
    // tierLimits geven we wel het werkelijke verbruik mee als Pro-preview,
    // zodat de UI-weekteller informatief blijft voor de admin.
    if (isAdminUser(userEmail)) {
      console.log(`🔓 Admin bypass active for ${userEmail}`)
      if (tool.tierLimits?.pro != null) {
        const resetPeriod = tool.resetPeriod || 'daily'
        const startOfPeriod = getStartOfPeriod(resetPeriod)
        const { data: adminUsage } = await supabase
          .from('scan_history')
        ...
        const used = adminUsage?.length || 0
        const tierLimit = tool.tierLimits.pro
        return {
          allowed: true,
          scansRemaining: Math.max(0, tierLimit - used),
          tier: 'pro',
          isPro: true,
          isAdmin: true,
          tierLimit,
    ...
    // ⭐ PAID USER - Lite & Pro subscribers
    // Tools met tool.tierLimits krijgen een tier-aware weekteller (geen bypass).
    // Tools zonder tierLimits behouden de unlimited bypass.
    ...
    const isActiveSub = ['active', 'canceling'].includes(profile?.subscription_status)
    // Legacy subscribers (tier null + active) gelden als Pro, consistent met
    // /api/auto-scan/route.js en /api/tracked-keywords/route.js.
    const stripeTier = profile?.subscription_tier || (isActiveSub ? 'pro' : null)
    const paidTierKey = stripeTier === 'lite' ? 'lite' : (stripeTier === 'pro' ? 'pro' : null)

    if (isActiveSub) {
      if (tool.tierLimits && paidTierKey && tool.tierLimits[paidTierKey] != null) {
        // Tier-aware weekly count voor deze tool.
        const resetPeriod = tool.resetPeriod || 'daily'
        const startOfPeriod = getStartOfPeriod(resetPeriod)
        ...
        const used = tierUsage?.length || 0
        const tierLimit = tool.tierLimits[paidTierKey]
        ...
        if (used >= tierLimit) {
          ...
          return {
            allowed: false,
            message: BETA_CONFIG.MESSAGES.beta_user_limit_reached(tool.name, tierLimit),
            scansRemaining: 0,
            tier: paidTierKey,
            isPro: paidTierKey === 'pro',
            isLite: paidTierKey === 'lite',
            tierLimit,
            resetPeriod,
      ...
      // Tools zonder tierLimits behouden de oude unlimited bypass.
      console.log(`⭐ ${(paidTierKey || 'pro').toUpperCase()} user bypass active for ${userEmail || userId}`)
      return { allowed: true, scansRemaining: 999, isPro: paidTierKey === 'pro', isLite: paidTierKey === 'lite', tier: paidTierKey || 'pro' }
    }
    ...
    // Free-users gebruiken tierLimits.free als die gedefinieerd is, anders limits.authenticated.
    const limit = tool.tierLimits?.free ?? tool.limits.authenticated
