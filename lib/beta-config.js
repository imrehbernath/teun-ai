...
export const BETA_CONFIG = {
  ...
  MESSAGES: {
    ...
    return `Je hebt je ${limit} gratis scans gebruikt. Maak een gratis account aan voor dagelijks 1 gratis scan!`
    },
...
export async function canUserScan(supabase, userId, toolName, ipAddress) {
  ...
  if (userId) {
    ...
    // ⭐ PRO USER BYPASS - Unlimited scans for paying subscribers
    ...
    if (['active', 'canceling'].includes(profile?.subscription_status)) {
      console.log(`⭐ Pro user bypass active for ${userEmail || userId}`)
      return { allowed: true, scansRemaining: 999, isPro: true }
    }
