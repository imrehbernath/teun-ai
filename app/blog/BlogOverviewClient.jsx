    ...
    <>
      ...
      {activeFilter !== 'all' && (
        <div className="mb-6 flex items-center justify-between">
          <p className="text-slate-500 text-sm">
            {filteredPosts.length} artikel{filteredPosts.length !== 1 ? 'en' : ''} in <span className="font-medium text-slate-700">{mainCategories.find(c => c.slug === activeFilter)?.name}</span>
          </p>
          <button
      ...
      {filteredPosts.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-slate-400 text-lg">Geen artikelen gevonden in deze categorie.</p>
        </div>
      ) : (
