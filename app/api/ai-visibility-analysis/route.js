...
async function sendSlackNotification(scanData) {
  ...
  try {
    const { companyName, companyCategory, primaryKeyword, totalMentions, scanId } = scanData;
      ...
      blocks: [
        ...
        },
        {
          ...
          elements: [
            {
              type: "mrkdwn",
              text: `Scan ID: \`${scanId}\` • ${new Date().toLocaleString('nl-NL')}`
            }
          ]
        },
        {
...
// ============================================
// AANPASSEN IN POST HANDLER (zoek deze code)
// ============================================

// NA deze regel (ongeveer regel 180-200):
...
// VOEG DIT TOE (direct na de insert):
if (savedScan) {
