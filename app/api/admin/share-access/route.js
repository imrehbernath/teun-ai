// app/api/admin/share-access/route.js
import { NextResponse } from 'next/server'
import { createClient, createServiceClient } from '@/lib/supabase/server'
import { Resend } from 'resend'
import crypto from 'crypto'

export const dynamic = 'force-dynamic'

const resend = new Resend(process.env.RESEND_API_KEY)

const ADMIN_EMAILS = ['hallo@onlinelabs.nl', 'imre@onlinelabs.nl']

const SHARE_TTL_DAYS = 30
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'https://teun.ai'

function generateShareToken() {
  return crypto.randomBytes(24).toString('hex')
}

function buildShareUrl(token) {
  return `${SITE_URL}/share/${token}`
}

// GET: Lijst alle shares (admin only)
export async function GET(request) {
  const authClient = await createClient()
  const { data: { user } } = await authClient.auth.getUser()

  if (!user || !ADMIN_EMAILS.includes(user.email)) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 403 })
  }

  // Service-client voor de feitelijke queries zodat RLS niet in de weg zit
  // (we hebben de admin-check hierboven al gedaan).
  const supabase = await createServiceClient()

  const { data: shares, error } = await supabase
    .from('shared_access')
    .select('*')
    .eq('owner_id', user.id)
    .order('created_at', { ascending: false })

  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  // Verrijk met scan counts en share URL per share. tool_integrations is de
  // bron voor AI Visibility data; chatgpt/google_ai_* tabellen geven extra info.
  const enrichedShares = await Promise.all((shares || []).map(async (share) => {
    const { count: integrationCount } = await supabase
      .from('tool_integrations')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', share.owner_id)
      .ilike('company_name', share.company_name)

    const { count: googleCount } = await supabase
      .from('google_ai_scans')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', share.owner_id)
      .ilike('company_name', share.company_name)

    const { count: overviewCount } = await supabase
      .from('google_ai_overview_scans')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', share.owner_id)
      .ilike('company_name', share.company_name)

    return {
      ...share,
      share_url: share.share_token ? buildShareUrl(share.share_token) : null,
      scan_counts: {
        integrations: integrationCount || 0,
        google_ai: googleCount || 0,
        ai_overviews: overviewCount || 0,
      }
    }
  }))

  return NextResponse.json({ shares: enrichedShares })
}

// POST: Maak nieuwe share aan, genereer token + URL, stuur optioneel email
export async function POST(request) {
  const authClient = await createClient()
  const { data: { user } } = await authClient.auth.getUser()

  if (!user || !ADMIN_EMAILS.includes(user.email)) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 403 })
  }

  const supabase = await createServiceClient()

  const body = await request.json()
  const { clientEmail, companyName, website, websiteId, note, ttlDays } = body

  if (!companyName) {
    return NextResponse.json({ error: 'Bedrijfsnaam is verplicht' }, { status: 400 })
  }

  const normalizedEmail = clientEmail ? clientEmail.toLowerCase().trim() : null
  const days = Number.isFinite(ttlDays) && ttlDays > 0 ? ttlDays : SHARE_TTL_DAYS
  const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString()
  const shareToken = generateShareToken()

  const { data: share, error } = await supabase
    .from('shared_access')
    .insert({
      owner_id: user.id,
      client_email: normalizedEmail,
      company_name: companyName,
      website: website || null,
      website_id: websiteId || null,
      note: note || null,
      permissions: ['view'],
      share_token: shareToken,
      expires_at: expiresAt,
    })
    .select()
    .single()

  if (error) {
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  const shareUrl = buildShareUrl(shareToken)

  let emailSent = false
  if (normalizedEmail) {
    try {
      await resend.emails.send({
        from: process.env.RESEND_FROM_EMAIL || 'noreply@teun.ai',
        to: normalizedEmail,
        subject: `${companyName} – jouw AI-zichtbaarheidsrapport staat klaar`,
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px;">
            <div style="text-align: center; margin-bottom: 30px;">
              <img src="${SITE_URL}/teun-ai-mascotte-mail.png" alt="Teun.ai" style="width: 60px; height: auto;" />
            </div>
            <h1 style="color: #1e1e3f; font-size: 22px; margin-bottom: 8px;">
              Jouw AI-zichtbaarheidsrapport is klaar
            </h1>
            <p style="color: #475569; font-size: 15px; line-height: 1.6;">
              We hebben de AI-zichtbaarheid van <strong>${companyName}</strong> geanalyseerd op ChatGPT, Perplexity, Google AI Mode en AI Overviews.
            </p>
            <p style="color: #475569; font-size: 15px; line-height: 1.6;">
              Open je rapport direct via onderstaande knop. Geen account nodig.
            </p>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${shareUrl}" style="background-color: #6366f1; color: white; padding: 14px 32px; text-decoration: none; border-radius: 10px; font-weight: 600; font-size: 15px; display: inline-block;">
                Bekijk rapport
              </a>
            </div>
            <p style="color: #94a3b8; font-size: 13px; line-height: 1.5; word-break: break-all;">
              Of kopieer deze link: <a href="${shareUrl}" style="color: #6366f1;">${shareUrl}</a>
            </p>
            <p style="color: #94a3b8; font-size: 12px;">
              Deze link is ${days} dagen geldig.
            </p>
            ${note ? `<p style="color: #64748b; font-size: 13px; background: #f8fafc; padding: 12px 16px; border-radius: 8px; border-left: 3px solid #6366f1;"><em>${note}</em></p>` : ''}
            <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 30px 0;" />
            <p style="color: #94a3b8; font-size: 12px; text-align: center;">
              Gedeeld via <a href="${SITE_URL}" style="color: #6366f1; text-decoration: none;">Teun.ai</a>
            </p>
          </div>
        `
      })
      emailSent = true
    } catch (emailError) {
      console.error('Failed to send invite email:', emailError)
    }
  }

  return NextResponse.json({
    success: true,
    share: { ...share, share_url: shareUrl },
    shareUrl,
    emailSent,
    message: normalizedEmail
      ? (emailSent ? `Link aangemaakt en gemaild naar ${normalizedEmail}.` : `Link aangemaakt. E-mail kon niet verstuurd worden; deel hem handmatig.`)
      : `Link aangemaakt. Kopieer hem hieronder.`
  })
}

// DELETE: Verwijder een share
export async function DELETE(request) {
  const authClient = await createClient()
  const { data: { user } } = await authClient.auth.getUser()

  if (!user || !ADMIN_EMAILS.includes(user.email)) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 403 })
  }

  const supabase = await createServiceClient()

  const { searchParams } = new URL(request.url)
  const shareId = searchParams.get('id')

  if (!shareId) {
    return NextResponse.json({ error: 'Share ID is verplicht' }, { status: 400 })
  }

  const { error } = await supabase
    .from('shared_access')
    .delete()
    .eq('id', shareId)
    .eq('owner_id', user.id)

  if (error) {
    console.error('Delete share error:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  return NextResponse.json({ success: true, message: 'Toegang ingetrokken' })
}
