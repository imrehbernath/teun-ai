...
export default function AdminSharesPage() {
    ...
    try {
      ...
      if (user) {
        ...
        if (websiteRows && websiteRows.length > 0) {
          setWebsites(websiteRows.map(w => ({ id: w.id, company_name: w.company_name, website: w.website_url || '' })))
        } else {
          ...
          for (const scan of (scans || [])) {
            ...
            if (key && !seen.has(key)) {
              seen.add(key)
              uniqueWebsites.push({ id: null, company_name: scan.company_name, website: scan.website || scan.website_url || '' })
            }
          }
  ...
  const handleShare = async () => {
    ...
    try {
      const res = await fetch('/api/admin/share-access', {
        ...
        body: JSON.stringify({ clientEmail: newEmail, companyName: selectedCompany, website: selectedWebsite, websiteId: selectedWebsiteId, note })
      })
      const data = await res.json()
      if (res.ok) {
        setSuccessMessage(data.message)
        setNewEmail(''); setSelectedCompany(''); setSelectedWebsite(''); setSelectedWebsiteId(null); setNote('')
        setShowAddModal(false)
        loadData()
      } else { alert(data.error || t('admin.somethingWrong')) }
    } catch (err) { alert(t('admin.errorPrefix') + err.message) }
    setSubmitting(false)
  }
  ...
  const handleRevoke = async (shareId) => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) { loadData() } else { alert(data.error || t('admin.deleteFailed')) }
    } catch (err) { alert(t('admin.errorPrefix') + err.message) }
  }

  const copyLoginLink = (email) => {
    navigator.clipboard.writeText('https://teun.ai/login')
    setCopied(email)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">{t('admin.noAccess')}</h1>
          <p className="text-slate-500">{t('admin.adminOnly')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {successMessage && (
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl flex items-start gap-3">
            ...
            <div><p className="text-green-800 font-medium">{successMessage}</p></div>
            <button onClick={() => setSuccessMessage('')} className="ml-auto text-green-400 hover:text-green-600"><X className="w-4 h-4" /></button>
          </div>
        )}

        <div className="mb-8 p-5 bg-blue-50 border border-blue-200 rounded-2xl">
          <h3 className="font-semibold text-blue-900 mb-3">{t('admin.howItWorks')}</h3>
        ...
        {shares.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-500 mb-4">{t('admin.noClients')}</p>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500 mt-3">
                      ...
                      <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{t('admin.shared')}: {formatDate(share.created_at)}</span>
                      {share.last_viewed_at && (
                        <span className="flex items-center gap-1.5 text-green-600"><Eye className="w-3.5 h-3.5" />{t('admin.viewed')}: {formatDate(share.last_viewed_at)}</span>
                      )}
                    </div>

                    {share.scan_counts && (
                      <div className="flex gap-2 mt-3">
                        {share.scan_counts.perplexity > 0 && <span className="px-2 py-0.5 bg-purple-50 text-purple-600 text-xs rounded">Perplexity: {share.scan_counts.perplexity}</span>}
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <button onClick={() => copyLoginLink(share.client_email)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition cursor-pointer" title={t('admin.copyLoginLink')}>
                      {copied === share.client_email ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                    </button>
                    <button onClick={() => handleRevoke(share.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition cursor-pointer" title={t('admin.revokeAccess')}>
            ...
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl">
              <div className="p-6 border-b border-slate-100">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold text-slate-900">{t('admin.addClientTitle')}</h2>
              ...
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5"><Mail className="w-3.5 h-3.5 inline mr-1" />{t('admin.clientEmail')}</label>
                  <input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder={t('admin.emailPlaceholder')} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none" />
                  <p className="text-xs text-slate-400 mt-1">{t('admin.emailHint')}</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5"><Building2 className="w-3.5 h-3.5 inline mr-1" />{t('admin.shareProject')}</label>
                  {websites.length > 0 ? (
                    ...
                    <input type="text" value={selectedCompany} onChange={(e) => setSelectedCompany(e.target.value)} placeholder={t('admin.companyPlaceholder')} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none" />
                  )}
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1.5">{t('admin.noteLabel')}</label>
                  <input type="text" value={note} onChange={(e) => setNote(e.target.value)} placeholder={t('admin.notePlaceholder')} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none" />
                </div>
              </div>
              ...
              <div className="p-6 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button onClick={handleShare} disabled={!newEmail || !selectedCompany || submitting} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 disabled:opacity-50 transition cursor-pointer">
                  {submitting ? (<><Loader2 className="w-4 h-4 animate-spin" /> {t('admin.sharing')}</>) : (<><Send className="w-4 h-4" /> {t('admin.grantAccess')}</>)}
                </button>
              </div>
            </div>
          </div>
