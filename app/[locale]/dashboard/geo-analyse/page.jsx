...
function GEOAnalyseContent() {
  ...
  // GEO Checklist label translation helper
  const cl = (id) => t('checklist.' + id)
  const clCat = (key) => t('checklistCat.' + key)
    ...
    for (let i = 0; i < uniquePages.length; i++) {
      ...
      try {
        ...
        const scanPromise = fetch('/api/geo-scan-page', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ url: pageUrl, locale })
        })
  ...
  // Save page scores to Supabase for GEO Optimalisatie DIY tab in dashboard
  const savePageScoresToDB = async (results) => {
    try {
      ...
      console.log(`✅ Saved ${entries.length} page scores to geo_audit_results`)
    } catch (e) {
  ...
  const steps = [
    ...
    { num: 4, label: t('step4'), icon: Search },
            ...
            <div className="space-y-6">
                ...
                <>
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                    <h3 className="font-semibold text-amber-900 mb-3 flex items-center gap-2">
                      <Shield className="w-5 h-5" /> {t('authorityManual')}
                    </h3>
                    <div className="grid sm:grid-cols-2 gap-2">
                      {GEO_CHECKLIST.autoriteit.items.map(item => (
                        <label key={item.id} className="flex items-center gap-2 bg-white rounded-lg p-2 cursor-pointer hover:bg-amber-100">
                          ...
                          />
                          <span className="text-sm text-slate-700">{cl(item.id)}</span>
                        </label>
                      ))}
                    </div>
                  </div>
              ...
              <div className="flex justify-between pt-4">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
                  {t('viewResults')} <ArrowRight className="w-5 h-5" />
                </button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">{t('geoResults')}</h2>
              ...
              {Object.keys(geoResults).length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  <div className="bg-slate-50 px-4 py-3 border-b border-slate-200">
                    <h3 className="font-semibold text-slate-800">{t('pageScores')}</h3>
                  </div>
