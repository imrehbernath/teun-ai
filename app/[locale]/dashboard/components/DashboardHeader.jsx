    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-2">{t('header.totalSites')}</p>
          <div className="flex items-end justify-between">
            <p className="text-4xl font-bold text-slate-900">{stats.totalSites}</p>
            <span className="text-xs text-green-600 bg-green-50 px-2 py-1 rounded-full font-medium">
              +{stats.totalSites} {t('header.thisMonth')}
            </span>
          </div>
        </div>
        ...
        {/* Average Score */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-2">{t('header.avgScore')}</p>
          <div className="flex items-end justify-between">
            ...
            <div className={`flex items-center gap-1 text-xs ${changeInfo.color} bg-slate-50 px-2 py-1 rounded-full font-medium`}>
              ...
              {changeInfo.prefix}{Math.abs(stats.scoreChange)} {t('header.vsLastMonth')}
            </div>
          </div>
        </div>
        ...
        {/* Total Scans */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-2">{t('header.totalScans')}</p>
          <div className="flex items-end justify-between">
            ...
            <span className="text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded-full font-medium">
              {stats.scansThisMonth} {t('header.thisMonth')}
            </span>
          </div>
        </div>
        ...
        {/* Best Performer */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          <p className="text-xs text-slate-500 uppercase tracking-wider font-medium mb-2">{t('header.bestPerformer')}</p>
