...
async function loadLatestScanFromDashboard() {
  try {
    console.log('📥 Loading latest scan from dashboard...');
    
   // Get auth token
      const { teun_token } = await chrome.storage.local.get(['teun_token']);

      console.log('🔑 Token exists:', !!teun_token);
      console.log('🔑 Token length:', teun_token?.length);
      console.log('🔑 Token preview:', teun_token?.substring(0, 20) + '...');
    ...
    const response = await fetch(`${getDashboardUrl()}/api/integrations/latest`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      console.warn('⚠️ Failed to fetch latest scan:', response.status);
      return;
    }
    ...
    if (data.success && data.integration) {
      ...
      console.log('✅ Latest scan loaded:', scan);
      ...
      if (scan.commercial_prompts && scan.commercial_prompts.length > 0) {
        ...
        showNotification(`✅ Laatste scan geladen: ${scan.commercial_prompts.length} prompts klaar!`, 'success');
      } else {
        console.log('ℹ️ No commercial prompts in latest scan');
      }
    } else {
      console.log('ℹ️ No previous scan found - user can enter manually');
    }
    
  } catch (error) {
    console.error('❌ Error loading latest scan:', error);
    // Fail silently - user can still manually enter data
  }
}
...
async function handleStartScan() {
  ...
  if (prompts.length > 20) {
    showError('Maximum 20 prompts per scan');
    return;
  }
...
function loadSamplePrompts() {
  const samples = [
    "Wat zijn de beste HBOT klinieken in Nederland?",
    "Welke bedrijven bieden hyperbare zuurstoftherapie aan?",
    "Hoe kies ik een goede HBOT behandeling?",
    "Wat zijn de kosten van hyperbare zuurstoftherapie?",
    "Waar kan ik HBOT therapie doen?"
  ];
  
  promptsTextarea.value = samples.join('\n');
  promptsTextarea.style.height = 'auto';
  promptsTextarea.style.height = promptsTextarea.scrollHeight + 'px';
}

// Add sample prompts button listener
document.getElementById('load-samples-btn')?.addEventListener('click', loadSamplePrompts);
